...
| `knot connect` | 运维客户端：只出不进，无 TUN，把本机端口转发到网络里（Mac App） |
...
连线上数据库调试的时候，你要的不是把笔记本变成网络的一个成员，而是**让本机的
某几个端口能读到线上的某几个端口** —— 就是 `ssh -L`，只不过隧道是 Reality。

`knot connect` 就是这个。它是第三种角色，把原本绑在一起的三个属性拆开了：
...
- **这份凭证等于「那台机器能访问的一切」。** 客户端的权限是「让某个节点替它去连
  某个地址」，所以笔记本丢了等于那些节点的网络暴露了。用完就在面板里吊销，
